...
def methodepas(pphi,pr0,pr02,vr,vr2,rc):
    ...
    for i in range(99): 
        rcb=rc[i]
        vr= vr+Fl(pr0,rho,U,H,a)/m*dt-Fd(pr0,mu,a,rcb,rho,Uf,H)/m*dt 
        vr2= vr2+Fl(pr02,rho,U,H,a2)/m2*dt-Fd(pr02,mu,a2,rcb,rho,Uf,H)/m2*dt              #Utilisation du PFD sur la particule 1
        pr0=pr0+vr*dt
        pr02=pr02+vr2*dt
...
def integrale(y,pphi,rho,U,H,a,mu,Uf,rc):
    for i in range(100):
        ...
        pr,p=y
        dydt=[p,Fl(pr0,rho,Q,H,a)-Fd(pr0,mu,a,rcb,rho,Uf,H)]
    return dydt

'''y0=[pr0,0.0]
sol1=odeint(integrale,y0,pphi,args=(rho,U,H,a,mu,Q,rc))
x1=(sol1[:,0])
plt.plot(pphi,x1)
plt.show()'''
